...
class CamRTL(depth:Int, tagWidth:Int, dataWidth:Int) extends Module{ /// TODO MultiIOModule? Maybe only for multiple channels...?
  ...
  // data array port
  val memaddr = Mux(hit, hitIdx, replIdx)
  val dataport = data(memaddr)
  
  // CAM operations
  when(io.in.en){
    // update replcement policy for hit
    when(hit){ rpolicy.access(setIndex, hitIdx) }
    ...
    when(io.in.wr){
      ...
      io.out.rdData := Mux(hit, dataport, DontCare)
    }
...
object CAM_Driver extends App {
  (new stage.ChiselStage).execute(args, Seq(stage.ChiselGeneratorAnnotation(() => new CAMModel(1024, 20, 32))))
}

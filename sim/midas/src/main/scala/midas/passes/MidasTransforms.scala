...
private[midas] class MidasTransforms extends Transform {
  ...
  def execute(state: CircuitState) = {
    ...
    val optionalTargetTransforms = if (p(GenerateMultiCycleRamModels)) Seq(
      ...
      new fame.LabelCAMModels,
      new ResolveAndCheck,
      new EmitFirrtl("post-wrap-sram-models.fir"),
      new fame.EmitFAMEAnnotations("post-wrap-sram-models.json"))
    else Seq()
    ...
    val xforms = Seq(
      ...
      new fame.EmitFAMEAnnotations("post-wrap-top.json")) ++

...
case class CamPortAnnotation(rt: ReferenceTarget, labelName: String) extends Annotation with FAMEAnnotation{
  def update(renames: RenameMap): Seq[Annotation] = {
    val renamer = RTRenamer.exact(renames)
    Seq(CamPortAnnotation(renamer(rt), labelName))
  }
  override def getTargets: Seq[ReferenceTarget] = Seq(rt)
}

...
/**
  * This labels a target Cam instance so that it is extracted and replaced with a separate model.
  */
case class CamModelAnnotation(target: BaseModule) extends chisel3.experimental.ChiselAnnotation {
  def toFirrtl: FirrtlCamModelAnnotation = {
    val parent = ModuleTarget(target.toNamed.circuit.name, target.parentModName)
    FirrtlCamModelAnnotation(parent.instOf(target.instanceName, target.name))
  }
}

case class FirrtlCamModelAnnotation(
  target: InstanceTarget) extends SingleTargetAnnotation[InstanceTarget] with FAMEAnnotation {
  def targets = Seq(target)
  def duplicate(n: InstanceTarget) = this.copy(n)
}
